...
# Flag to track existing ambulance in left lane
ambulance_in_left_lane = False
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction):
        ...
        global ambulance_in_left_lane

        # Check for existing ambulance before creating a new one (only in left lane)
        if vehicleClass == 4:
            if not ambulance_in_left_lane:  # Check if ambulance flag is not set
                ambulance_in_left_lane = True  # Set flag if new ambulance is created
            else:
                self.kill()  # Don't create this ambulance if another exists in left lane
                return
        ...
        simulation.add(self)
                ...
                # Reset ambulance flag when the left lane is empty
        #if direction == 'left' and len(vehicles[direction][lane]) == 0:
         #   ambulance_in_left_lane = False
...
def generateVehicles():
   last_ambulance_time = time.time()  # Track time of last ambulance generation
   while True:
      vehicle_type = random.randint(0,4)
      if vehicle_type != 4:
        lane_number = random.randint(1, 2)  # Corrected to include lane 0
      ...
      elif time.time() - last_ambulance_time >= 30:
        lane_number = 1  
        last_ambulance_time = time.time()  # Update last ambulance generation time
        temp = random.randint(0,99)
        direction_number = 2  # Set direction to left for ambulance
        dist = [25,50,75,100]
        if(temp<dist[0]):
            direction_number = 0
        elif(temp<dist[1]):
